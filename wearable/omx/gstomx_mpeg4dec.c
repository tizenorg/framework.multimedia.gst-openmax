...
#ifdef DIVX_DRM /* we do not support divx now */
...
print_tag (const GstTagList * list, const gchar * tag, gpointer data)
{
  ...
  omx_base_filter = GST_OMX_BASE_FILTER (data);
      ...
      if (strcmp (gst_tag_get_nick(tag), "DRM DivX") == 0) {
        if (self->drmContext == NULL) {
          ...
          if (init_divx_drm (self)) {
            omx_base_filter->is_divx_drm = TRUE;
...
#endif /* we do not support divx now */

static void
instance_deinit (GstElement * element)
{
  GstOmxMpeg4Dec *self;
  self = GST_OMX_MPEG4DEC (element);

  GST_WARNING_OBJECT (self, "mpeg4 dec deinit");
  ...
  GST_WARNING_OBJECT (self, "mpeg4 dec end");
}
...
instance_private_value_init(GstElement * element)
{
  ...
  omx_base_filter->gomx->compression_format = OMX_VIDEO_CodingMPEG4;
}
